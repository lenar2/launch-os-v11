...
if [[ -z "${LAUNCH_OS_TEST_DATABASE_URL:-}" ]]; then
  if ! command -v docker >/dev/null 2>&1; then
    echo "docker is required when LAUNCH_OS_TEST_DATABASE_URL is not set" >&2
    exit 1
  fi
...
"${PYTHON_BIN}" -m pytest -m business_outcomes tests/integration
